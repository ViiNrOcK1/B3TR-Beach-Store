...
import { WalletButton, useWallet, useThor } from '@vechain/dapp-kit-react';
import { Clause, Units, Address, ABIItem, ABIFunction } from '@vechain/sdk-core';
...
function Store({ showManageForm, setShowManageForm }: StoreProps) {
  ...
  const { account, connect, signer } = useWallet();
  const thor = useThor();
  ...
  const { data: balanceData } = useQuery({
    ...
    queryFn: async () => {
      ...
      // @ts-ignore
      const balance = result.decoded[0] as string;
  ...
  const handlePurchase = async (product: Product) => {
    ...
    if (!account) {
      try {
        ...
        if (!connectedAccount) {
          setPaymentStatus('No wallet selected. Please connect your wallet.');
          console.error('No wallet selected');
          alert('Please open the VeWorld extension, create or select a wallet, and ensure Mainnet is selected. See https://docs.vechain.org/veworld for help.');
          setSelectedProduct(null);
          return;
        }
      } catch (err: any) {
        setPaymentStatus('Failed to connect wallet: ' + err.message);
        console.error('Wallet connection failed:', err);
        alert('Failed to connect to VeWorld. Please ensure the extension is installed, a wallet is selected, and Mainnet is active. See https://docs.vechain.org/veworld for help.');
        setSelectedProduct(null);
        return;
      }
    }
  ...
  const handleB3TRPayment = async () => {
    ...
    try {
      ...
      } else if (error.message.includes('node') || error.message.includes('CORS')) {
